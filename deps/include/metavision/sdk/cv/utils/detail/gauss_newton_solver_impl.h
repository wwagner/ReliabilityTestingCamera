...
template<typename Impl, typename Scalar, int DIM,
         template<typename I, typename S, int D> typename LeastSquaresProblemBase>
bool solve(LeastSquaresProblemBase<Impl, Scalar, DIM> &problem, const TerminationCriteria<Scalar> &term_criteria) {
    Report<Scalar> report;
    GaussNewton::solve(problem, report, term_criteria);

    return report.state != Report<Scalar>::State::ARITHMETIC_FAILURE &&
           report.state != Report<Scalar>::State::EXTERNAL_FAILURE;
}
} // namespace GaussNewton
} // namespace Metavision
