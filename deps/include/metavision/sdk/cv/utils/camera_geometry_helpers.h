...
inline void camera_to_undist_norm(const Ti &pt3_cam, To &pt2_norm) {
    ...
    const T invz            = T(1) / T(get_z_element(pt3_cam));
...
/// @warning The output v2_undist_norm could be inf if \f$v3^{cam}_z + pt3^{cam}_z = 0\f$.
template<typename Ti, typename To>
inline void vector_camera_to_undist_norm(const Ti &v3_cam, const Ti &pt3_cam, To &v2_undist_norm, To &pt2_undist_norm,
    ...
    const T inv_d                  = T(1) / (get_z_element(pt3_cam) + get_z_element(v3_cam));
    const T inv_d2                 = inv_d * T(1) / get_z_element(pt3_cam);
    const T gradx_d                = -inv_d2 * get_x_element(pt3_cam);
    const T grady_d                = -inv_d2 * get_y_element(pt3_cam);
    ...
    get_x_element(pt2_undist_norm) = get_x_element(pt3_cam) / get_z_element(pt3_cam);
    get_y_element(pt2_undist_norm) = get_y_element(pt3_cam) / get_z_element(pt3_cam);
...
/// @brief Computes the normalization maps used to convert a point from distorted image coordinates to undistorted
/// normalized coordinates
/// @tparam T The floating point type used
/// @tparam T1 The output matrix type used
/// @param[in] camera_geometry The CameraGeometryBase instance used to map the point
/// @param[out] mapx LUT giving the undistorted normalized x coordinate of the input distorted image point (integer
/// coordinates)
/// @param[out] mapy LUT giving the undistorted normalized y coordinate of the input distorted image point (integer
/// coordinates)
template<typename T, typename T1>
inline void get_normalization_maps(const CameraGeometryBase<T> &camera_geometry, T1 &mapx, T1 &mapy) {
    ...
    allocate(mapx, img_size(1), img_size(0));
    allocate(mapy, img_size(1), img_size(0));
...
///
/// The maps are NxM matrices where NxM is the destination sensor's resolution.
///
...
inline void compute_camera_transfer_maps(const CameraGeometryBase<T> &src_cam_geometry,
    ...
    allocate(mapx, dst_size(1), dst_size(0));
    allocate(mapy, dst_size(1), dst_size(0));
