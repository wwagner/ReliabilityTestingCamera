...
public:
    ...
    struct Parameters {
        ...
        std::uint32_t search_radius_ = 3; ///< Radius (in pix) in which matches are looked for for each support point
        ...
        std::uint32_t nb_directional_axes_ =
            8; ///< Number of pre-computed axes to consider to quantize the normal of an edge
    };
...
private:
    /// @brief Samples and tracks support points
    /// @return True if the tracking has succeeded
    bool process_internal();

    /// @brief Pre-computes the relevant pixels for all @ref Parameters::nb_directional_axes_ edge orientations in a
    /// circle of radius @ref Parameters::search_radius_ to speed up the search of each support point's match in the
    /// timesurface
    void compute_support_point_offsets();

    /// @brief Checks if the computed pose is a valid pose
    bool is_pose_valid();

    /// @brief Tracks the support point in the time surface using precomputed pixel offsets
    /// @param[in] target_ts Oldest timestamp allowed for a valid match
    /// @param[in] pt_img Coordinates of the support point in the time surface
    /// @param[in] vec_img Search direction (i.e. the edge's normal)
    /// @param[out] ts_match Matched candidate's timestamp
    /// @param[out] match If the tracking succeeds, the matched candidate
    /// @return True if the tracking as suceeded, false otherwise
    bool track_support_point(const timestamp &target_ts, const Eigen::Vector2f &pt_img, const Eigen::Vector2f &vec_img,
                             timestamp &ts_match, Eigen::Vector2f &match);
    ...
    size_t ts_idx_;
    std::vector<Eigen::Vector2i> axes_offsets_;
    int diameter_;
};
} // namespace Metavision
