...
/// @param[in] vec_img_search Search direction (i.e. edgelet's normal)
...
bool track_support_point_both_directions(const MostRecentTimestampBuffer &time_surface, const timestamp &ts_target,
                                         const cv::Matx21f &pt_img, const cv::Matx21f &vec_img_search,
                                         unsigned int search_radius, std::vector<cv::Matx21f> &match_candidates,
                                         timestamp &ts_match, int &match_idx);

/// @brief Tries to match an edgelet's support point (i.e. a point sampled along an edgelet) in a time surface
///
/// Matching candidates are sampled along the direction of the edgelet's normal and within a given search radius
/// of a given width. A support point is then matched to a matching candidate when its timestamp is more recent
/// than a target one.
///
/// @param[in] time_surface Time surface in which matches are looked for
/// @param[in] ts_target Oldest timestamp allowed for a valid match
...
/// @param[in] vec_img_search Search direction (i.e. edgelet's normal)
/// @param[in] search_radius Radius in which matches are looked for
/// @param[in] width Distance from the normal in which matches are still looked for
/// @param[out] match The matched candidate if the tracking succeeds
/// @param[out] ts_match Matched candidate's timestamp
/// @return True if the tracking as succeeded, false otherwise
bool track_support_point_both_directions_width(const MostRecentTimestampBuffer &time_surface,
                                               const timestamp &ts_target, const cv::Matx21f &pt_img,
                                               const cv::Matx21f &vec_img_search, unsigned int search_radius,
                                               unsigned int width, cv::Matx21f &match, timestamp &ts_match);
...
/// @param[in] pt_img Coordinates of the support point in the time surface
/// @param[in] vec_img_dir Expected edge's direction
/// @param[in] search_radius Radius in which matches are looked for
...
bool track_support_point_on_expected_slope(const MostRecentTimestampBuffer &time_surface,
                                           const PlaneFittingFlowEstimator &flow_estimator, const cv::Matx21f &pt_img,
                                           const cv::Matx21f &vec_img_dir, unsigned int search_radius,
