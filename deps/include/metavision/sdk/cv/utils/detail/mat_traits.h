...
template<typename T>
inline void allocate(T &mat, int rows, int cols, dynamic_opencv_matrix_tag) {
...
template<typename T>
inline void allocate(T &mat, int rows, int cols) {
    static_assert(!std::is_void<typename mat_traits<T>::value_type>::value, "Unsupported matrix type!");

    typename mat_traits<T>::category c;

    return detail::allocate(mat, rows, cols, c);
}
