...
private:
    ...
    /// @tparam InputTrackIt An iterator type over a track of type @ref LineParticleTrack
    /// @param roi_size Size of the output image
    /// @param ts Detection timestamp
    /// @param begin Begin iterator to the particle tracks to display
    /// @param end Past-end iterator to the particle tracks to display
    template<typename InputTrackIt>
    void add_new_tracks(const cv::Size &roi_size, timestamp ts, InputTrackIt begin, InputTrackIt end);

    void add_new_track(const cv::Size &roi_size, const LineParticleTrack &track);
...
void LineParticleTrackDrawingHelper::draw(timestamp ts, cv::Mat &output_img, InputTrackIt begin, InputTrackIt end) {
    ...
    add_new_tracks(output_img.size(), ts, begin, end);
...
template<typename InputTrackIt>
void LineParticleTrackDrawingHelper::add_new_tracks(const cv::Size &roi_size, timestamp ts, InputTrackIt begin,
                                                    InputTrackIt end) {
